...
any \tcp connection previously created using  \texttt{tcpls\_connect()} and joined using
\texttt{tcpls\_handshake()}. Properties over the streams can be negotiated, such as the
failover mode (global to all streams) or \textit{Coupled Streams}, that let the
application uses the same interface \texttt{tcpls\_send()}  and \texttt{tcpls\_receive()}
...
$O(log(n))$ for $n$ elements in the heap. Checking the min sequence number in
the heap is in $\Theta(1)$ and retrieving is also in $O(log(n))$. The
performance of our multipath bandwidth aggregation is evaluated in
Sec.~\ref{sec:bwaggr}.
