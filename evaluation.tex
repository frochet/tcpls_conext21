...
to match the ones we use internally in \tcpls. This fixes fragmentation and 
unnecessary copies of received \tls records that could occur, and improves by
+$\approx \%40$ the
implementation's throughput which our \tcpls 
prototype avoids by design by offering a new transport API preventing the
application developers to make choices heavily affecting the throughput. With
the implemented fixes, we can observe that \tcp/\tls reaches 10.3~Gbps 
with a 1500-byte MTU and 12.6~Gbps with a 9000-byte MTU. Both leverages using 
...
described in Sec.~\ref{sec:multipath} to support failover.
...
network, and we assume the NICs are a component of the network and not part of the
user's stack).
