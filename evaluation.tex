...
to how much \tcpls should read from \tcp due to the requirement of having a complete recor to decrypt it. We may want to read
from \tcp more than the \textit{nbytes} instructed by the caller, or that could
cause fragmentation. % adversely affecting the %throughput.  
...
Further works may also evaluate multiple \tcpls and
\quic sessions at the same time. The effect of multiple parallel streams for both protocol
stacks would also be interesting to compare for a fairness evaluation over
realistic Internet-like experimental conditions.
