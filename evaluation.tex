...
starting our fork. We also manually configured the receiving buffer size 
to match the maximum read buffer size of \tcpls. This fixes fragmentation and 
unnecessary copies of received \tls records that could occur, which our \tcpls 
prototype avoids by design. We can observe that \tcp/\tls reaches XXX~Gbps 
with a 1500-byte MTU and XXX~Gbps with a 9000-byte MTU. Both leverages using 
...
\tcp/\tls reaches XXX~Gbps with a 1500-byte MTU and XXX~Gbps with a 9000-byte 
MTU. \tcpls has thus a similar throughput than \tcp/\tls when using the base 
...
\vspace{-0.5cm}
  \caption{\tcpls raw performance and comparison with state of the art.
    ...
    offering a superset of protocol features and similar flexibility.}
  \label{fig:perf}
\end{figure}
...
obtained when performing such a migration during a file transfer.% using the 
%API 
...
\vspace{-0.5cm}
  ...
  \todo{Legende bleu "CUBIC", orange "Vegas -> eBPF CUBIC"}
