...
\label{sec:bwaggr}
We compare the bandwidth aggregation capability of \tcpls to the state of the
art \mptcp. Both
protocols run in the same Mininet topology with two IP paths, 
each 
offering 25~Mbps and a latency of 10~ms. The experiment consists in transfering
a file over a single path and then enabling the second one after $5$ seconds. 
\mptcp automatically detects the new path once we enable the second network interface on the Client. For
...
decides when to migrate, and we expose a simplistic code flow to perform it). In
this experiment, we use a Mininet network~\cite{handigol2012reproducible} composed of a client and a server, both
dual-stacks. The IPv4 and IPv6 paths are completely disjoint in this emulated
network. 
We configure the
bandwidth on each path to be 30~Mbps, a round-trip-time of 40~ms for the IPv4 path,
and a round-trip-time of 80ms for the IPv6 one. Our application downloads
...
if a \tcp connection is not already established, the application has the choice \todo{why choice?}
