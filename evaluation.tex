...
developers a transport API preventing them. % adversely affecting the 
%throughput. 
...
MTU. The small advantage of \tcpls at an MTU of 1500 bytes may be 
...
which could further improve the throughput with an MTU of 9000 bytes. 
...
The results obtained show that \tcpls compares favorably
...
option, which is set to $250~ms$ in our experiment. This value can also be
chosen by the application according to specific use cases.
...
obtained when performing such a migration during a file transfer with TCPLS. We 
do not compare TCPLS to MPTCP and QUIC as none of their implementation offer 
this feature directly to applications. % using the 
%API 
...
They have two differences. First, for MPTCP, there is a delay before it 
