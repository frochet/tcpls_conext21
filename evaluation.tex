...
received the entire content. \tls/\tcp record fragmentation is provided by TLS
...
control algorithms were implemented in the different QUIC stacks, but setting a
...
\label{sec:bwaggr}
We compare the capability of \tcpls to the state of the art \mptcp. 
...
network. 
Both protocols run in the same topology with two IP paths available, 
each offering 25 Mbps and a latency of 10 ms.
The experiment consists to transfering
a 60MB file over a single path and then enabling the second one at $5$ seconds. For
...
decide when to migrate, and we expose a simplistic code flow to perform it). 

In this experiment, we rely on the Mininet topology from Sec.~\ref{sec:bwaggr}.
We set a
bandwidth of 30~Mbps on each path, a round-trip-time of 40ms for the v4 link,
...
of doing a \texttt{tcpls\_connect()} that would make a $\tcp$ connection and
return a \texttt{transport identification} to the application. The second option
is to  directly call the required \texttt{tcpls\_handshake()} configured with handshake properties $TFO$,
and announcing a \join over the v6 connection id. Then, the creation of a new stream
...
first path finishes to flush its data while the second path already started to
