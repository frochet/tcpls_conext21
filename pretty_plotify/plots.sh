...
python3 plot_perf.py --trace $input/tcpls_jumbo_aes128gcmsha256.log $input/tcpls_default_mtu_aes128gcmsha256.log $input/tcpls_jumbo_failoveron_aes128gcmsha256.log $input/tcpls_mtu1500_failoveron_aes128gcmsha256.log $input/tcpls_mtu1500_aggregation.log $input/tls_jumbo_aes128gcmsha256_readbuf_fixed.log $input/tls_default_mtu_aes128gcmsha256_fixed.log $input/quicly_gso_default_mtu.log $input/quicly_jumbo_pmtu1460.log $input/quicly_default_mtu.log $input/msquic_jumbo_aes128gcmsha256.log $input/msquic_default_mtu.log $input/mvfst_gso_default_mtu.log $input/mvfst_jumbo_pmtu1460.log $input/mvfst_default_mtu.log --label "TCPLS \large{\textbf{tso} on \textbf{mtu} 9000}" "TCPLS \large{\textbf{tso} on}" "TCPLS \large{\textbf{tso} on \textbf{mtu} 9000 \textbf{failover} on}" "TCPLS \large{\textbf{tso} on \textbf{failover} on}" "TCPLS \large{\textbf{tso} on \textbf{aggregation on}}" "TLS/TCP \large{\textbf{tso} on \textbf{mtu} 9000}" "TLS/TCP \large{\textbf{tso} on}" "Quicly \large{\textbf{gso} on}" "Quicly \large{\textbf{gso} on \textbf{mtu} 9000}" "Quicly" "Msquic \large{\textbf{mtu} 9000}" "Msquic" "mvfst \large{\textbf{gso} on \textbf{pacing} on}" "mvfst \large{\textbf{gso} on \textbf{mtu} 9000 \textbf{pacing} on}" "mvfst \large{\textbf{pacing} on}" --oname $output/perf_analysis --ext $1
