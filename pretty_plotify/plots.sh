...
python3 plot_perf.py --trace $input/mvfst_default_mtu.log $input/mvfst_gso_default_mtu.log $input/mvfst_jumbo_pmtu1460.log $input/msquic_default_mtu.log $input/msquic_jumbo_aes128gcmsha256.log $input/quicly_default_mtu.log $input/quicly_gso_default_mtu.log $input/quicly_jumbo_pmtu1460.log $input/tls_default_mtu_aes128gcmsha256.log $input/tcpls_default_mtu_aes128gcmsha256.log $input/tls_jumbo_aes128gcmsha256.log $input/tcpls_jumbo_aes128gcmsha256.log $input/tcpls_mtu1500_failoveron_aes128gcmsha256.log $input/tcpls_jumbo_failoveron_aes128gcmsha256.log --label "mvfst \large{\textbf{pacing} on}" "mvfst \large{\textbf{gso} on \textbf{pacing} on}" "mvfst \large{\textbf{gso} on \textbf{mtu} 9000 \textbf{pacing} on}" "Msquic" "Msquic \large{\textbf{mtu} 9000}" "Quicly" "Quicly \large{\textbf{gso} on}" "Quicly \large{\textbf{gso} on \textbf{mtu} 9000}" "TLS/TCP \large{\textbf{tso} on}" "TCPLS \large{\textbf{tso} on}" "TLS/TCP \large{\textbf{tso} on \textbf{mtu} 9000}" "TCPLS \large{\textbf{tso} on \textbf{mtu} 9000}" "TCPLS \large{\textbf{tso} on \textbf{failover} on}" "TCPLS \large{\textbf{tso} on \textbf{mtu} 9000 \textbf{failover} on}" --oname $output/perf_analysis --ext $1

echo "Plotting aggregate dual..."
python3 plot_aggregate_dual.py --goodput $input/mptcp_client_goodput_aggregation.log $input/tcpls_client_goodput_aggregation.log --tcpdump $input/mptcp_client_tcpdump_pruned_aggregation.log $input/tcpls_client_tcpdump_pruned_aggregation.log --event_at 18:42:08.472578 20:20:20.967599 --event_text "Interface UP" "APP decision to use new path" --event_pos 15 5 -i 0.05 --oname $output/aggregate_dual --ext $1
