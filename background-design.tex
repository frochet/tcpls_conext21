...
For instance, for \tcp, it is \tcp segments that are split in two with only
a small part dedicated to control data.
\tls is more flexible with \tls records of different types and lengths.
In \quic, the basic unit is a \quic packet which can be composed of several and
different types
...
encryption and decryption operations can be performed atomically and that they 
enable a zero-copy codepath.
Any \tcpls control information that is not relevant to the application is 
%always encrypted and 
located at the end of the record so that adjusting the offset when decrypting
the \tcpls records can overwrite them and achieve a zero-copy codepath.
...
\tcpls from \tls past the handshake. This ease its deployment and
hardens censorship attempts. Whenever a middlebox block the client
