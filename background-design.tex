...
from TLS 1.3 \texttt{AppData} records, leading to middlebox compatibility. %and 
...
communication in \tcpls. Section~\ref{design.start} explains how a \tcpls session is managed. 
Section~\ref{sec:transport-services} then shows what are the new transport 
services this combination can offer ({\small{\textit{RQ2}}}) by showing
how \tcpls uses \tcpls records to provide modern transport services such as 
multiplexing, connection migration, and a novel multipath approach.
...
decrypting the \tcpls records, the receiver can overwrite them and
achieve zero-copy within a contiguous allocated memory. 
This specific protocol
design choice allow \tcpls implementations to easily offer
zero-copy data delivery through a simple API. In
comparison, \quic's design force developers to achieve zero-copy by offering a 
more complex API delivering potentially fragmented data.% (i.e., multiple 
%pointers and sizes).
