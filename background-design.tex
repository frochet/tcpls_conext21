...
with transport-level control data allowing us to support a range of transport services.
...
For instance, \tcp segments that are split in two with only
the header dedicated to control data.
\tls is more flexible since it supports \tls records of different types and lengths.
...
located at the end of the record so that by adjusting the offset when decrypting
the \tcpls records the receiver can overwrite them and achieve a zero-copy codepath.
...
\tcpls extends these messages to negotiate some of the new
transport services it offers during the handshake.
...
Since \tls messages are conveyed in the \tcp payload, they are not space constrained like existing \tcp options.  
...
\todo{Detecting proxies arrive de façon un peu bizarre}
...
\tcpls's handshake extensions, it could choose to enable \tcpls after the 
handshake using \tcpls control records, provided that the server announced 
\tcpls support.
The censor would not be able to 
trivially distinguish this behaviour from classical \tls, hardening potential 
attempts for censoring applications using \tcpls.
The server handshake extensions are 
encrypted and thus cannot be target by such attacks.
