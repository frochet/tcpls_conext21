...
flexible manner. Some \tcpls control information may sometimes be packaged
within a record containing Application Data, such as a sequence number for
bandwidth aggregation. Any control information is always encrypted and located
at the end of the record for performance reasons: it guarantees the availability
of a zero-copy codepath after decryption. In our implementation, a contiguous
...
packet. Moreover, the fact that DATA frames can be sandwitched within control frames 
prevents a contiguous memory blob to be given to the application in zero-copy.
Assuming everything else being equal, these facts lower the expected performance
