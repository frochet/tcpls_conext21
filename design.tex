...
further \tcp connections. It contrasts to \mptcp for which any observer of the
initial handshake would own the required information to join the \mptcp
connection. The new transport capabilities of \tcpls may also require stronger
situational attacker models. One example would be preventing different IP
networks from colluding and linking other \tcp connections belonging to the same
\tcpls session. The \tcpls initial handshake and the Join mechanism can be
enhanced to support this goal, offering encrypted tokens in the initial
handshake. One could then spend these tokens for each Join handshake,
...
%the bandwidth of several \tcp connections.
