...
\todo{parler de secure control channel ?}
...
\todo{Il faut expliquer comment les stream id sont choisis pour garantir l'unicité}
...
involve a complete decryption as all TLS 1.3 AEAD ciphers use an Encrypt-then-MAC construct. \todo{citer rfc7366 ? ou les références de ce rfc ?}
...
We leverage this mechanism to implement new transport services
...
An attacker having observed the key exchange during the \tcp handshake can associate
...
Appendix~\ref{app:failover}.\todo{Let see if we can bring it back in the text later}

\textbf{Application Connection Migration}. \tcpls also enables the application
to trigger a connection migration, for instance based on application-level 
metrics qualifying its experience over the current path. %expected to be used 
%over
...
%The decision to migrate from a healthy network to another one is an 
%application one.
...
An interactive game could use different streams for chat messages and player's commandes.  Also, an HTTP server could choose
...
\todo{OB: paragraphe ci-dessous pas clair}

Applications that benefit from more bandwidth for when transmitting a single 
application object can leverage \tcpls \textit{coupled streams}. \todo{This 
