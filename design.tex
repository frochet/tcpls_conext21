...
modes, including connection failover and bandwidth aggregation.
...
to solves this \texttt{``connection join''} problem in a much more secure
...
scenario). The client sends a \textsc{ClientHello} containing the \tcpls
...
(SESSID($\alpha$) in Fig.~\ref{fig:join-example}). This identifier uniquely
