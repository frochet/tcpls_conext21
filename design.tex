...
bits, \tls computes the cryptographic nonce given to the underlying \todo{manque un mot} by
...
\todo{minimum or maximum?} block size). We thus observe that 7 bits among those 32 bits are
...
\todo{FR: why both ? this failover and failover ok exchange. If so, we should menton this and say that it works over the secure channel}
