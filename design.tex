...
\begin{figure}
	\centering
	\begin{bytefield}[bitheight=\widthof{ab}]{32}
...
Figure~\ref{fig:aead-iv} illustrates how the AEAD nonce is computed for a 
given \tcpls record of a \tcpls stream. First the left-most bits of the IV derived from the \tls handshake
is summed with the \tcpls Stream ID. Then the right-most bits are XORed with the
stream record sequence number. Each \tcpls stream has a separate record sequence number space,
