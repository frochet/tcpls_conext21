...
streams over the different \tcp connections. While connection migration moves
\tcpls streams from a connection to another, stream steering allows the
application to distribute the streams over the different \tcp connections of a
\tcpls session. This provides a service similar to Multipath QUIC
\cite{de2017multipath,draft-liu-multipath-quic-02} when several QUIC streams are
assigned to paths.
...
The choice of assigning to the \tcpls streams to the \tcp connections is left to
...
maintaining the ability to process each stream in a zero-copy manner.
