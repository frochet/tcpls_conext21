...
\subsection{Multiplexing in \tcpls}\label{sec:datastreams}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\todo{Choose one way to represent the stream id}
...
advantages when combined with connection migration and multipath.
...
keys used~\cite{chatterjee2011another}. To overcome this, we propose an
...
\todo{Do a figure to explain the derivation, TLS 1.3 IV construct vs TCPLS IV derivation}
\todo{Update the following explanation with the figure}
...
\subsection{Connection Migration}\label{sec:multipath}
...
\paragraph*{2) A More Flexible Connection Migration}
...
\subsection{Improving \tcp's Extensibility}\todo{Move this to the beginning of the design as a simple example of tcpls records}
