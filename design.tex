...
transport  services: stream multiplexing (Sec.~\ref{sec:datastreams}), connection  migration (Sec.~\ref{sec:multipath}), and bandwidth aggregation
(Sec.~\ref{sec:multipath}). We leverage the \tcpls records to multiplex concurrent encrypted bytestreams. By extending the \tls handshake, \tcpls allows joining several \tcp connections to a single \tcpls session to support
...
These features are built atop the \tcp bytestream, and as such they avoid  middleboxes interference that modify the \tcp header.
...
\begin{figure}[!t]
	...
	\begin{bytefield}[bitheight=\widthof{abc}]{32}
		...
		\bitbox{12}{\tcpls Stream ID} & \bitbox[]{8}{} & \bitbox{12}{\small
		Stream record sequ.}
	\end{bytefield}
...
\tls 1.3 wire format must be preserved to avoid possible middleboxes
interference. Second, the resulting nonce must be unique for each record of
the \tcpls session in order to maintain the security properties of AEAD.
...
all \tcpls streams given a unique Stream ID and sequence number, which is the
main security concern. The Stream ID space is split between the client and the
...
It does not involve a complete decryption as all \tls 1.3 AEAD
...
connection.\footnote{The worst case is when the sender schedules records of $N$
  ...
  record of the current stream the Stream ID of the following record.%, thanks
  to \tcp ordered delivery %of \tcpls data and control records.
}
...
probability $2^{-33}$. What this result means is that an unvalid payload would
check correctly after an unachievable number of failed decryptions (and would
decrypt to a pseudo-random payload anyway).
...
\textbf{Failover}.
When a \tcp connection fails, e.g., due to middleboxes or network outages, \tcpls leverages its joining mechanism to recover the session over another \tcp connection. This is particularly useful on multihomed devices that often move out of reach of a given access network, for instance a smartphone moving away from a Wi-Fi access point, but also on all devices that suffer from middleboxes disrupting \tcp connections, for instance a firewall introducing \tcp \rst on idle \tcp connections. Fig.~\ref{fig:design-failover} illustrates
...
performing Application Connection Migration %, which plays
...
application to attach streams to separate \tcp connections to avoid head-of-line blocking. %by placing them on separate \tcp connections.
...
Using socket options such as \texttt{tcp\_info}, an application can retrieve
useful statistics about the performance of the underlying \tcp connections (e.g.
RTT, congestion window, \ldots). %If required, it is also possible to A more
advanced application could also define \tcpls records to actively probe a
connection, e.g., with an echo/request record to actively measure delays, or
retrieve information from the remote host, e.g. by retrieving the remote host's
\texttt{tcp\_info} structure.

Coupled streams can be used when performing Application Connection Migration to
smoothly transition from one network path to another without impacting the
application goodput.

\subsection{Security and Information Leakage}

Compared to \mptcp, \tcpls achieves a secure Join protocol in which
cryptographically authenticated endpoints can join an existing session with
further \tcp connections. It contrasts to \mptcp for which any
observer of the initial handshake would own the required information to join the
\mptcp connection. The new transport capabilities of \tcpls may also require
situational stronger attacker models. One example would be to prevent different
IP networks to collude and link together different \tcp connections belonging to
the same \tcpls session. The \tcpls initial handshake and the Join protocol can
be enhanced to support this goal, offering encrypted Session IDs in the initial
handshake. We could then spent those session ids for each Join handshake,
effectively acting as both a session identifier and a cookie. Further more
session ids could be generated and shared upon successful Joins.
