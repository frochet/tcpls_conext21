...
Stream Multiplexing is a modern transport service that has been part of recent
...
stream consists of a sequence of \tcpls stream data records encrypted with its
cryptographic context. Each stream is attached to one \tcp connection.
...
operation is occasional and negligible upon regular usage of the
...
\textbf{Joining \tcp connections}. \tcpls leverages its secure records and 
...
exchanged in cleartext. The server sends cookies encrypted and they can only be
used once by the client.
%(i.e.,
...
over another \tcp connection. To achieve such a \emph{break-before-make}, \tcpls sends stream-level record
acknowledgments for the records that have been received
over each stream. Upon reception of Stream-level acknowledgments, the sender can
manage its sending buffer and remove acknowledged encrypted records. Upon network outage, \tcpls moves the
...
transmitted again over a functional one. To resynchronize the session (i.e., the
implicit encryption/decryption sequence numbers), the
failover protocol makes the client sends the sequence number of the first record
...
the default context and sent within the new \tcp connection. Receiving this protocol message, the server now knows
the sequence number to decrypt some record arriving next on the connection that
read the failover message. Either the server already processed that record, and can safely
ignore it and increase the sequence number, or the server did not yet seen that
record, and can decrypt and process it. The same protocol message and operations
...
present in Section~\ref{sec:eval_failover} an analysis of recovery speed against
different type of outages. We also give a
...
with opening, attaching and closing \tcpls stream protocols to move the
application data to a new connection.
...
\textbf{Coupling streams for aggregated bandwidths}. Recall that each stream is
attached to only one \tcp connection. Applications that benefit
...
the record. The sender can schedule \tcpls records of application
object over these
streams and benefit from their aggregated bandwidth by sending data over the two
stream using a strategy that fits the application usage. The receiving application
reads the application object in order, as \tcpls handles the reordering when
processing the decrypted records.
...
\subsection{Finishing a \tcpls Connection}\label{design.closing}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todo{OB: pas critique pour moi, ou alors il faut comparer MPTCP et QUIC}

\todo{What's left here that does not belong to the Failover or implementation section?}
...
%When a stream is attached for the first time to a \tcp connection, this connection becomes active.
