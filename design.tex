...
middleboxes interferences that modifies the \tcp header.
...
interference. Second, each record of the \tcpls session must has a unique
...
These two operations guarantee that every record of every \tcpls streams has a
unique nonce.
%This combination of
...
before succeeding with probability $2^{-33}$. Also, note that in case the adversary succeeds in decrypting the packet, it is very likely it will lead to a plain payload non-conforming to the \tcpls protocol.
%note that succeeding the
%decryption is very likely to result in a decrypted payload non-conforming to
%the \tcpls protocol.

%What this result means is that an
%unvalid payload would
